...
- 状态：原方案本地实现及离线验证完成；2026-09-22 决策材料第一版功能回归通过，状态查询提交已按 ADR-043 转入应用层后台用例；真实模型收益及重复性能样本仍待 S09。
...
程序通过 `decision_context` 构造助手当前决策材料，而不是序列化执行恢复状态：保留目标、有效约束、活跃事项、必要依赖状态、当前就绪项和未结算操作/分派的身份及状态；按 admission 标明 answer、execute、execute_and_answer 或 reconcile。已完成/取消事项仅提供计数，完整 effects/evidence/dispatches、完成事项描述和 receipt 不重复注入。详情通过当前请求限定的 `read_request_state` 查询；近期工具结果优先复用。原始选择范围继续精确保留，不猜测或重绑定后来 UI 的选择。精确 ID、许可和完成证据仍由代码核验，模型材料不替代 checkpoint，也不能恢复节点或重新授权。

每份决策材料以 `request_decision_context` 包装，携带独立正整数 `state_seq`；同请求最大序号替代较小序号及所有旧版无序号 `current_request_state`，不按消息位置判断。序号在内容实质变化或配置重建时递增，不使用 request revision 冒充进度版本；压缩保留最大序号，附件恢复保留原值。普通追加不改写旧字节；每条材料有独立身份及分组，状态 A→B→A 不复用旧组。无效/重复序号、当前 digest 与最大序号不一致直接阻止准备，不自动重编号或回退全量状态。
...
逻辑顺序为 `固定规则 → S1 → S2 → … → Sn → 本版本必要决策材料 → 保留的近期完整历史 → 后续新消息/决策材料`；工具 schema 仍经供应商独立参数提供。首版无摘要时省略空链。下一次压缩仅在 Sn 后插入 S(n+1)，保留最高 state_seq 的材料并替换被本次摘要覆盖的原文段；保留的旧摘要块不改写。尾部残留旧材料的序号较小或无序号，不能覆盖当前决策材料。
...
4. 决策材料只在模型相关字段发生实质变化时追加。目标修订、待决变化、任务终态和必要运行身份有明确序号；重复唤醒、内部进度 sequence、已完成 receipt、会话保存 revision、诊断时间戳不导致新材料。详细进度和执行记录按需查询。固定计划仍由执行器推进；用户已选择本版保留计划完成后的模型解释回合，不根据未经定义的交付类型猜测是否省略回答。
...
2026-09-22 第一版另提供 `read_request_state`：只读当前 admission 的 items/effects/evidence/dispatches/selection，
字符页最大 8000；offset > 0 必须携带上一页 digest，版本变化明确报错，不拼接不同版本或自动重试。
查询材料不产生许可、验收证据或用户指令。当前实现后台读取、前台交付时重验 admission 与整个 section digest，
并在同一服务锁内追加及保存回执。功能隔离验证已通过，但大 section 的前台读取/序列化会阻塞 UI。
后续按 ADR-043 与综合计划迁移到应用层结果裁决及后台提交；该范围已获用户授权，不能删掉校验规避此问题。
...
缓存使用助手独立 profile，翻译 profile 不变。助手在已识别的 OpenAI 官方端点/模型上使用稳定 key 与自动前缀缓存，不进入仅标 system 且关闭隐式历史断点的 explicit 模式；Anthropic 使用已核验的自动历史缓存。未知端点不猜参数。首次切换旧 explicit 请求布局或新增工具 schema 允许一次配置/前缀重建，之后普通追加保持稳定。不能把本地前缀相同率当成真实缓存命中。
