...
**FR6.10 AI翻译/润色结果报告系统** — *2026-08-25 | 状态: 已实现（canonical report） | 优先级: P1*: 系统 SHALL 在 AI 翻译或润色完成后生成唯一的 canonical `ReportSnapshot`，应用内交互式对话框、JSON、CSV、Excel 和持久化历史 SHALL 只消费该快照。报告 SHALL 覆盖翻译、后处理和独立校对润色，支持单次和批量场景，不得维护第二套统计或 legacy DTO 报告投影。
...
**FR6.9 独立校对润色入口** — *2026-08-27 | 状态: 已实现 | 优先级: P0*: 系统 SHALL 在 AI 翻译窗口中提供「润色」预设，对已翻译条目默认以一次 LLM「校对润色」同时完成对照原文纠错、术语语境处理和表达优化；不得默认把检测、修复、润色和裁决拆成多次重复扫描。旧的独立多阶段链 SHALL 作为严格或自定义策略保留。

  - **FR6.9.1 预设与输入**: 选择「润色」SHALL 加载以已有译文为输入、关闭翻译、开启校对润色的推荐配置；无译文条目默认跳过。用户随后修改的作用域和阶段配置优先于预设。
  - **FR6.9.2 一次校对润色**: 标准策略 SHALL 在同一请求中向模型提供原文、当前译文、上下文和相关术语，并要求按“语义纠错 → 术语一致 → 流畅与风格优化 → 合格时保持不变”的优先级返回最终译文。默认响应 SHALL 只要求稳定条目标识和最终译文，不要求自然语言修改理由、模型自报置信度或模型自定裁决，以减少输出 Token。
  ...
  - **FR6.9.6 预览确认**: 启用预览时 SHALL 逐条展示原文、当前译文和最终校对润色候选并允许接受或拒绝；严格策略可额外展示检测、修复与裁决详情。正式 collection 只能在确认后一次提交。
  - **FR6.9.7 进度与异常**: 标准策略 SHALL 以一个「校对润色」阶段展示连续批次进度并支持暂停/停止；严格策略按实际启用阶段展示。选中范围为空或均无译文时 SHALL 给出原因，任一失败候选不得覆盖原译文。
  - **FR6.9.8 可追踪报告**: GUI、历史与导出报告 SHALL 记录处理策略、实际阶段、请求失败和最终候选；标准策略不伪造不存在的检测问题、修复详情、信心度或裁决结果。
  - **FR6.9.9 Smart Assistant 后处理预设对齐** — *2026-08-27 | 状态: 已实现*: `run_postprocess` 无参数调用 SHALL 默认使用内置润色预设的 `combined` 校对润色，而非隐式执行全部严格阶段。工具 SHALL 支持选择内置 translate/polish/mixed 预设或具名自定义工作流，并允许显式覆盖作用域、润色强度、策略、严格阶段及共享并发/Token 额度；服务商、模型、端点、凭据和术语路径仍继承全局配置。显式传入旧 `phases` 且未指定策略时 SHALL 兼容解释为 `strict`；`phases` 与 `combined` 同时出现 SHALL 拒绝并说明冲突。工具报告、任务元数据和日志 SHALL 记录最终生效的配置来源、策略、阶段、作用域和运行额度。
