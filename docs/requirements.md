...
**FR6.9 独立校对润色入口** — *2026-08-26 | 状态: 已实现（综合 QA 通过） | 优先级: P0*: 系统 SHALL 在 AI 翻译窗口中提供「润色」预设，对已翻译条目默认执行“对照原文检测错误 → 针对性修复 → 表达润色 → 最终校验/裁决”，而不是仅改善文风。用户可按 FR5.13 覆盖阶段配置形成仅检测、仅纠错、纯润色或完整校对润色流程。

  - **FR6.9.1 预设与输入**: 选择「润色」SHALL 加载以已有译文为输入、关闭翻译、开启校对润色的推荐配置；无译文条目默认跳过。用户随后修改的作用域和阶段配置优先于预设。
  - **FR6.9.2 错误检测范围**: 默认校对 SHALL 对照原文、当前译文、上下文和术语，检测错译、漏译、增译、否定/数字/专名错误、语义偏差、术语不一致、占位符/标签/引号等格式错误以及明显语法问题。
  - **FR6.9.3 针对性修复**: 启用自动修复时，系统 SHALL 仅针对检测问题生成修复候选并保留问题—修复对应关系；未检测到问题时不得为纠错目的改写。修复后的候选 SHALL 成为后续润色输入，不得让润色重新从旧译文开始并覆盖修复成果。
  - **FR6.9.4 表达润色**: 启用表达润色时，系统 SHALL 按 light/moderate/aggressive 强度及 all/passed/has_issues 范围优化流畅度、风格和语境，同时保持原文语义、术语、占位符和格式标记。原译文已正确自然时允许原样返回。
  - **FR6.9.5 最终校验与裁决**: 启用裁决时 SHALL 对最终候选给出 pass/reject/pending。预览关闭时只自动提交 pass；pending、reject、未解决严重错误、低信心、阶段失败或取消 SHALL 保留正式译文并进入报告/人工审核。
  - **FR6.9.6 预览确认**: 启用预览时 SHALL 逐条展示原文、当前译文、检测问题、纠错候选、润色候选、最终译文、裁决与信心度，允许接受或拒绝；正式 collection 只能在确认后一次提交。
  - **FR6.9.7 进度与异常**: 执行 SHALL 显示当前阶段和条目进度并支持暂停/停止。选中范围为空或均无译文时 SHALL 给出原因；任何单阶段失败不得以成功润色覆盖原译文。
  - **FR6.9.8 可追踪报告**: GUI 与 Excel 报告 SHALL 区分检测问题、应用的纠错、表达润色、未解决问题和裁决结果，并记录 requested/executed/skipped stages 及跳过原因。
