...
> **规划状态（2026-08-24）**：FR24 仍未实现；需求、ADR 与 Plan 已根据已完成的 FR25/FR26 重新对齐。FR25 提供可注入的 View/Presenter/Binding 边界，FR26 已冻结开始中心、Workbench、AI 快速运行、操作计划、任务中心、命令搜索和可访问性关键路径。FR24 SHALL 在这些现有交互合同上迁移视觉与基础服务，不得恢复旧信息架构或旧多模态链。

**FR24.1 统一设计令牌** — *状态：草稿 | 优先级：P0*：GUI SHALL 通过版本化、只读的语义设计令牌表达颜色、字体、间距、圆角、边框、尺寸和交互状态。业务组件不得依赖具体主题的十六进制颜色；成功、警告、错误、运行状态、Stage、标签、差异与译文状态等业务语义 SHALL 在不同主题中保持含义稳定，且不得仅靠颜色传达关键信息。

**FR24.2 内置主题模式** — *状态：草稿 | 优先级：P0*：首期 SHALL 提供浅色、深色和跟随操作系统三种模式。用户切换后，当前应用内已打开窗口、对话框、自定义绘制区域与后续新建组件 SHALL 收敛到同一主题 revision；选择 SHALL 通过统一应用配置持久化。系统主题检测不可用时，跟随系统模式 SHALL 使用可诊断的稳定回退主题，不得反复切换或阻断启动。

**FR24.3 应用级主题服务** — *状态：草稿 | 优先级：P0*：GUI 进程 SHALL 只有一个主题状态所有者，负责解析主题、生成不可变快照、应用 Qt Palette/框架样式并在 revision 实际变化时发出一次变更通知。普通控件 SHALL 优先通过 Qt Palette 与字体继承机制获得样式；主题颜色不得由全局 QSS 维护；自定义绘制和无法继承的组件 SHALL 使用同一快照，不得各自读取配置、扫描窗口树或维护第二套主题状态。

**FR24.4 公共组件与适配边界** — *状态：草稿 | 优先级：P0*：框架 SHALL 为按钮、输入控件、卡片、对话框、表格、标签、工具提示、空状态、进度、通知及焦点状态定义统一约定，并为富文本、Markdown、Delegate、消息气泡和其他自定义绘制提供主题适配合同。迁移 MAY 渐进进行，但已迁移组件不得重新引入裸颜色常量或整段局部主题 QSS。

**FR24.5 设置、预览与恢复** — *状态：草稿 | 优先级：P1*：设置界面 SHALL 允许选择主题模式、预览结果和恢复默认值。设置写入失败时 SHALL 保持当前有效主题并提示失败；无效或已移除的主题 ID SHALL 回退到内置默认主题并保留诊断。首期不提供完整主题编辑器、任意皮肤导入、主题市场、复杂动效或布局密度编辑。

**FR24.6 主题扩展合同** — *状态：草稿 | 优先级：P1*：框架 SHALL 预留带 schema/version、稳定 theme ID、元数据、完整令牌集和资源声明的主题提供者接口，使未来编辑器、外部主题包或品牌主题可在不修改业务组件的前提下接入。注册前 SHALL 校验缺失令牌、非法值、兼容版本、资源预算与 ID 冲突；校验失败 SHALL 原子拒绝整个主题并回退，不允许半应用。首期 MAY 只注册内置提供者，不要求用户安装或编辑扩展主题。

**FR24.7 无障碍基础合同** — *状态：草稿 | 优先级：P1*：公共组件 SHALL 规定可见焦点、键盘可达、可访问名称/描述、状态非纯颜色表达以及文本与关键图形的最低对比度。框架 SHALL 尊重操作系统字体与 DPI 缩放；主题扩展不得绕过对比度及焦点可见性校验。首期要求基础合同和关键路径迁移，不要求一次性重写所有历史窗口。

**FR24.8 国际化基础合同** — *状态：草稿 | 优先级：P1*：用户可见字符串 SHALL 逐步迁入统一翻译资源与 locale 服务，公共组件不得固化中文文案。首期 SHALL 建立资源加载、缺失翻译回退、locale 持久化和组件重译合同；完整文案迁移 MAY 分阶段进行。语言切换若不能安全、低成本地即时刷新全部历史组件，MAY 明确要求重启后生效，但不得出现同一窗口内语言状态不一致。

**FR24.9 渐进迁移与兼容** — *状态：草稿 | 优先级：P0*：框架 SHALL 支持按组件族渐进迁移，并提供旧样式兼容期、使用清单和完成门禁。迁移期间新旧组件必须可共存且功能行为不变；主题故障不得影响解析、翻译、保存、同步等业务数据。移除兼容层前 SHALL 证明生产 UI 不再依赖旧颜色/QSS 路径。

**FR24.10 可诊断失败与安全回退** — *状态：草稿 | 优先级：P0*：主题资源解析、扩展注册、系统模式检测或运行时应用失败时，系统 SHALL 保留最后一个完整有效快照或回退到内置浅色主题，记录不含敏感路径的结构化诊断，并保持 UI 可操作。重复选择当前有效主题 SHALL 幂等，不得触发重复全局重绘。

**FR24.11 FR25/FR26 接入与交互保真** — *状态：草稿 | 优先级：P0*：UI Foundation SHALL 从 `ui/app.py` 与各功能 composition root 显式注入，只影响 View、公共组件和绘制适配器；Theme/Locale revision SHALL 与 project/projection/render/run/operation revision 相互独立。主题或 locale 变化不得改变开始中心目的地、Action Catalog intent、引导强度、表格行身份与编辑状态、AI RunSpec、操作计划/确认 token、TaskRuntime 状态、恢复/重试能力或任何本地/远端副作用。FR24 SHALL 复用 FR26 已建立的开始中心、Guidance、Workbench 切片、AI config/scope/run/result、operation plan、任务中心、命令搜索、拖放与无障碍合同；已退出生产组合的 Step1/Step3 等兼容模块只有在可达性审计证明仍被生产入口使用时才进入视觉迁移范围。
