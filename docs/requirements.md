...
**状态：FR30.1～FR30.15 的核心链路、独立派生摘要与长期请求归档已实现（2026-09-12）；真实模型语料评估未执行。FR30.16～FR30.22 为本次规划草稿，尚未实现。** 关联 FR12、FR13、FR19、FR20；已实施架构见 [ADR-040](adr/040-assistant-user-request-lifecycle.md)，上下文演进提议见 [ADR-041](adr/041-assistant-context-compaction.md)。本节扩展用户请求管理，不改变既有翻译项目快照语义。
...
**FR30.2 输入接纳与关系判定**：输入 SHALL 在被确认接纳后可靠保留；连续快速发送不得丢失较早问题。关系判定 SHALL 覆盖新建、追问、补充约束、继续、取消及替换，支持同一消息的多个操作。目标不明确时只澄清受影响的操作；用户没有表达取消时，换题不得自动取消旧请求。
...
**FR30.7 完整历史与模型上下文**：系统 SHALL 分别管理持久化消息记录和本轮模型输入；裁剪、压缩或切换模型不得删除已保存历史。当前目标、有效约束、未完成项和用户待决事项应在预算内优先保留；请求摘要记录来源和覆盖范围，不能覆盖权威任务状态或用户授权。
...
**FR30.10 可见与可纠正**：聊天区 SHALL 能查看当前目标、其他未完成目标、正在等待的原因和关联结果，提供继续、暂停推进、取消及纠正请求归属的入口。自动分类不确定或错误时允许纠正；已执行工作不能通过修改显示归属绕过所有权或修订检查。
...
**FR30.13 模型与程序职责**：模型 SHALL 只提出有来源的请求变更和问答覆盖声明；程序负责分配正式身份、验证、持久化并返回接纳结果，执行状态依据真实运行事件更新。控制协议和业务执行入口分离，模型不能通过任意状态写入宣称完成，截断或被中断的回答不能满足验收项；重试提案不能因模型改写编号而重复接纳。界面应区分主动停止生成、新消息技术中断、暂停推进与取消请求，用户主动停止后不自动恢复该请求。
...
实施状态（2026-09-12）：用户已授权实施 FR30.16～FR30.22，生产请求已接入稳定追加和独立语义摘要；旧确定性摘录仅保留兼容读取与基线测试。相关离线回归通过，真实语义质量、缓存收益及重复性能样本仍待评估。完整证据、请求归档和显式离线附件维护继续有效；不引入云端调度、多会话无人值守自主推进、任意项目回滚或依赖语义向量检索的恢复。

<a id="fr30-stable-context"></a>

#### FR30.16：稳定的模型上下文

同一请求在模型、固定规则、工具及相关配置未改变且未压缩时，普通问答和工具续跑 SHALL 保留已发送上下文并追加新材料，不因超过固定轮数重新移动历史窗口。目标、约束、状态变化 SHALL 可被模型及时识别，不原地重写过去的事实。模型可见材料与真实用户输入分开，运行事件不得变成新用户命令。归属纠正、配置变化或协议不兼容确需重建时 SHALL 记录原因。目标、计划、checkpoint、授权、取消与完成证据规则继续遵守 FR30.1～FR30.15。
