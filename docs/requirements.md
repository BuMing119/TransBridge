...
**状态：核心链路、独立派生摘要与长期请求归档已实现（2026-09-12）；真实模型语料评估未执行**。关联 FR12、FR13、FR19、FR20；架构见 [ADR-040](adr/040-assistant-user-request-lifecycle.md)。本节扩展用户请求管理，不改变既有翻译项目快照语义。
...
当前范围：独立派生摘要已包含阈值触发、后台生成、持久化、来源/版本校验和模型上下文读取；默认确定性摘录，不要求每个请求额外调用模型。已结束且无未决副作用的旧请求可归档并读取完整证据；无引用附件提供显式离线维护，默认仅预览，验证当前与备份引用后才能删除。首期不引入云端调度、多会话无人值守自主推进、任意项目回滚或依赖语义向量检索才能工作的请求恢复。
