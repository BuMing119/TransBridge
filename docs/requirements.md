...
**FR5.18 可切换译名方案与非破坏性术语适配** — *2026-09-06 | 状态: 通用定位与 AI 可见选择已实现（高级差异/绑定管理待续） | 优先级: P1*: 系统 SHALL 允许同一 Project/内容 Variant 只维护一份项目译文，并通过选择具有独立身份和版本的“译名方案”，快速预览、检查、翻译和导出采用不同专名约定的结果。译名方案可用于本体汉化、地区或社群译名、团队/发行规范、系列旧译与新译、整合包兼容等场景；切换 SHALL NOT 复制、批量覆盖或重写项目译文，除显式条目特例外，不同成品仅可在受控术语出现位置产生差异。技术实现继续使用 `TerminologyProfile` 契约，不把产品名称迁移解释为数据迁移。

  - **FR5.18.1 独立业务维度**: 译名方案 SHALL 独立于 Project 的内容 Variant。内容 Variant 继续表达默认版、润色版、实验版等正文状态；同一内容 Variant MAY 切换多个译名方案生成适配预览或成品，不得以复制完整 Variant 作为方案的权威模型。
  - **FR5.18.2 共同正文不变式**: 切换、预览或导出术语配置档 SHALL NOT 修改共同正文、条目 revision、stage、标签、来源证据或人工润色状态，也不得使 Project 因单纯切换配置档进入正文脏状态。配置档 A→B→A 后，共同正文及 A 的派生结果 SHALL 与首次选择 A 时一致。
  - **FR5.18.3 逻辑术语与配置档译名**: 同一来源术语 SHALL 具有跨配置档稳定的逻辑身份；每个配置档分别保存该术语的目标译名、状态和可选备注。配置档间译名差异属于预期适配结果，不得被报告为同一配置档内的同名异译冲突。
  - **FR5.18.4 受控术语出现位置**: 系统 SHALL 只在能够追溯到逻辑术语的受控出现位置应用配置档译名，不得对普通中文字符串执行无边界全文替换。长短术语重叠、同一中文译名对应多个来源术语、译名重复出现、人工改写或无法唯一定位时 SHALL 保持原文不变并产生待确认项。
  - **FR5.18.5 非破坏性派生译文**: 配置档适配结果 SHALL 是从共同正文、受控术语位置、配置档固定版本和显式特例得到的派生视图。派生结果 MAY 用于工作台显示、AI/校对上下文、质量检查和写出成品，但不得反向覆盖共同正文或冒充一次人工正文编辑。
  - **FR5.18.6 配置档选择与恢复**: 用户 SHALL 能创建、复制、重命名、选择和停用配置档，并查看当前选择、配置档版本、术语数量、缺失映射、受影响条目和特例数量。删除或停用当前正在使用、被运行快照引用或被历史成品引用的配置档 SHALL 被阻止或转为保留历史可读性的停用操作，不得产生悬空引用。
  - **FR5.18.7 解析优先级与隔离**: 术语解析 SHALL 按“当前配置档的条目特例 → 当前配置档的插件作用域术语 → 当前配置档普通术语 → 明确声明为跨配置档共用的模组术语”的顺序决定结果。一个配置档缺少译名时 SHALL 显示未适配，不得静默使用另一配置档、其他内容 Variant 或不兼容 legacy 来源的译名。
  - **FR5.18.8 方案特例**: 当某套译名约定需要量词、语序、搭配或上下文调整而不能只替换术语时，用户 SHALL 能为指定方案和条目保存显式完整译文特例。特例只影响该方案和该条目，必须可识别、可比较、可恢复为跟随项目译文，并计入差异和应用报告。
  - **FR5.18.9 编辑意图**: 用户编辑派生译文中的普通文本时，系统 SHALL 将其表达为共同正文编辑；编辑受控术语时 SHALL 允许用户明确选择修改当前配置档术语或仅创建当前条目特例。系统不得根据最后编辑位置静默猜测并批量修改所有配置档。
  - **FR5.18.10 历史译文识别**: 对已有普通译文，系统 MAY 基于原文术语命中和当前配置档译名执行受控位置识别。只有来源术语、目标译名和位置均可唯一证明时才可自动建立绑定；重叠、多义、目标缺失、非标准译法或位置不唯一 SHALL 进入待确认，不得自动改写正文。用户取消、失败或重复执行识别时不得留下部分正文修改或重复绑定。
  - **FR5.18.11 新翻译绑定**: 新翻译、润色、混合和自定义 AI 任务 SHALL 在页签外的固定任务上下文区与运行摘要中显示当前译名方案，并与工作台共享同一个 Project/Variant 选择。AI 界面 SHALL 明确区分“术语来源”和“译名方案”；任务 SHALL 在预检时固定当前内容 Variant、方案及版本，并在全部批次、重试、恢复、检查和报告中继续使用该固定组合。运行期间切换或应用方案更新只影响之后创建的任务。
  - **FR5.18.12 校对与一致性检查**: 术语一致性检查 SHALL 针对当前配置档验证受控术语和译文，不得用其他配置档的译名产生误报。检查 MAY 报告缺失绑定、缺失映射、配置档内冲突和特例偏离，但 SHALL 区分“共同正文问题”“配置档问题”和“条目特例”。
  - **FR5.18.13 版本与发布**: 配置档的正式发布版本 SHALL 不可变，并记录父版本、内容摘要、发布时间、映射和特例变化。修改发生在草稿中；回退 SHALL 通过发布新版本实现，不修改或删除历史版本。共同正文版本与配置档版本拥有独立生命周期，回退任一方不得隐式回退另一方。
  - **FR5.18.14 可复现成品**: 写回插件、XML、JSON 或其他成品前 SHALL 固定 Project、内容 Variant 及 revision、配置档及版本、共同术语版本和特例集合。相同固定输入重复生成 SHALL 得到相同派生译文；生成过程中任一绑定输入变化时不得产生混合配置档成品。
  - **FR5.18.15 导出标识与追溯**: 成品结果、运行报告和可追溯元数据 SHALL 记录配置档稳定身份、用户可读名称、固定版本和内容摘要。文件名 MAY 使用配置档显示名，但不得依赖可重命名显示名作为稳定身份。没有明确配置档或配置档不可用时，要求术语适配的导出 SHALL 被阻止并说明原因。
  - **FR5.18.16 配置档差异视图**: 用户 SHALL 能比较任意两个配置档版本，查看不同译名、缺失映射、受影响条目、配置档特例和无法安全派生的条目。差异统计 SHALL 来自与预览/导出相同的固定输入，不得分别重新推算出互相矛盾的结果。
  - **FR5.18.17 切换反馈与性能**: 选择译名方案后 SHALL 在 500 毫秒内同步更新工作台、AI 配置窗口的当前身份和可见反馈；当前可见条目的适配译文 SHOULD 在 500 毫秒内更新。大型 Project 的全量影响统计 MAY 在后台渐进完成，但 SHALL 保持界面响应、支持取消或被下一次切换替代，且不得调用 LLM、Embedding 或网络服务。单次主线程工作不得超过 FR5.16.34 的响应预算。
  - **FR5.18.18 原子切换与失败恢复**: 配置档读取失败、版本损坏、映射无效或派生计算失败时 SHALL 继续保留上一套完整可用视图，并说明失败配置档和影响；界面不得出现部分条目属于旧配置档、部分条目属于新配置档的混合状态。重试成功后再以单一边界切换当前视图。
  - **FR5.18.19 空值与不完整配置档**: 空配置档可以创建和编辑，但选择后未映射术语 SHALL 被明确统计。预览 SHALL 保留共同正文；导出含未适配受控术语时 SHALL 给出条目级影响和结果导向确认，不得将另一配置档的译名当作补全结果。
  - **FR5.18.20 ParaTranz 与外部术语边界**: 配置档与 ParaTranz 或外部术语库同步时 SHALL 使用显式、独立且可追溯的目标映射。多个配置档不得按最后同步顺序写入同一远端术语权威并互相覆盖；无法保真表达配置档身份、作用域或特例的目标 SHALL 跳过相应内容或要求用户确认有损映射。
  - **FR5.18.21 兼容迁移**: 既有 Project 升级后 SHALL 保持项目译文、内容 Variant、术语版本和 AI/写回行为可用。系统 MAY 为现有项目建立一个默认译名方案，但不得把 `TermEntry.variants`、Project Variant 显示名或 legacy 术语来源静默重解释为译名方案；无法建立历史术语绑定时保持原译文不变。
  - **FR5.18.22 缓存与失效**: 派生预览和影响统计 MAY 缓存，但缓存身份 SHALL 覆盖共同正文 revision、内容 Variant、配置档版本、共同术语版本、绑定和特例。清除缓存后结果必须一致；任一语义输入变化时旧缓存不得继续显示或进入导出。
  - **FR5.18.23 范围外**: 首版不自动扫描用户游戏目录或互联网识别已安装汉化，不通过机器翻译补齐配置档，不对无法唯一证明的历史译文执行强制替换，不自动重写因术语变化需要调整语序的整句，不默认一次批量生成全部配置档成品，也不在多个 Project 间传播未经确认的配置档修改。
  - **FR5.18.24 依赖与兼容**: 本需求依赖 FR5.13.4、FR5.16、FR5.17、FR6.2、FR7.9.4、FR8 的 Project/Variant 权威状态和 ADR-027 的规范术语模型。新配置档上下文必须进入 AI 运行、断点恢复、校对、报告、写回和导出的既有一致性边界，但不得把术语派生责任塞入 PyQt View、格式 adapter 或现有超出职责门禁的 `TermDatabaseManager`。
  - **FR5.18.25 从术语来源创建译名方案**: 用户 SHALL 能在项目术语工作台的“译名方案”区域选择一个项目动态词库、已绑定 ParaTranz 术语、已配置本地来源或新浏览的 JSON、CSV、Excel 文件，将其当前读取结果作为一次性快照创建为新的译名方案。AI 翻译任务只 SHALL 展示当前方案并提供前往术语工作台的入口，不得成为长期方案资产的唯一创建入口。生成结果 SHALL 以当前已发布项目术语的有效决定为完整骨架：命中且无歧义的术语采用来源译名，未命中、来源冲突或作用域不明确的术语保持当前项目译名，来源独有条目不得静默扩张项目术语权威。预览 SHALL 显示采用、保持、冲突和来源独有数量，并明确创建后不再跟随来源变化；没有可用项目术语版本、来源为空、来源不可用或读取失败时不得创建任何方案。创建成功的方案 SHALL 立即发布并出现在工作台与 AI 的共享选择器中，只有用户明确选择时才设为当前方案。网络和大文件读取 SHALL 在工作线程执行，不阻塞 Qt 事件循环。

  **FR5.18 已确认产品假设**:

  - 共同正文是唯一可编辑的默认译文；配置档切换是非破坏性派生，不写回共同正文。
  - 不同译名方案原则上只允许术语受控位置不同；无法满足该原则的少量内容使用显式方案特例。本体汉化适配是首个模板，而不是能力边界。
  - 当前配置档选择影响预览、之后创建的 AI/校对任务和成品导出；已启动任务继续使用冻结版本。
  - 历史普通译文只在映射可唯一证明时自动建立术语绑定，其他情况由用户复核。

  **FR5.18 验收场景**:

  - 两个配置档只有 40 个术语不同，切换后只有包含这些受控术语或显式特例的派生条目发生变化，共同正文及其 revision 完全不变。
  - A→B→A 与连续重复选择 A 均得到相同预览，不发生二次替换或文本累积损坏。
  - A 与 B 对同一逻辑术语使用不同译名时，配置档比较显示差异，但两边均不产生配置档内冲突。
  - B 缺少一个已绑定术语时，预览保留共同正文并显示未适配；系统不使用 A 或 legacy 术语填补。
  - 一个来源句包含长短重叠术语，历史识别无法唯一定位时只产生待确认项，不修改译文或建立错误绑定。
  - 修改共同正文的普通文本后，所有配置档派生结果都包含该修改；修改 B 的术语映射或条目特例只影响 B。
  - AI 任务启动后切换 A→B，该任务全部批次、恢复和报告仍记录并使用 A 的固定版本，新任务才使用 B。
  - 导出开始后配置档、共同正文或特例发生变化时，不产生半 A 半 B 的成品；旧快照继续安全完成或导出整体失效并要求重试。
  - 配置档版本回退后共同正文不回退；共同正文恢复历史版本时配置档历史和当前选择不变。
  - 旧项目迁移后正文、Variant 和原术语行为保持可用；没有可靠绑定的历史译文保持原样，并可在之后执行复核。
...
- **FR9.3.8 stop_task** (`write`): 控制当前会话内的翻译、润色或计划任务。参数：`task_id: str`（可选）、`action: stop|pause|resume`（默认 stop）、`all_tasks: bool`（默认 false）。省略 ID 仅选择唯一活跃顶层任务，多任务时要求明确目标；全部操作须显式指定 all_tasks 并确认。明确单任务停止不重复确认，返回实际状态，区分取消中和已取消；不得操作其他会话任务。
...
**FR19.7 模式验证与迁移** — *状态：已确认 | 优先级：P0*：持久化文件 SHALL 带有可验证的模式版本。系统 SHALL 验证类型、必需字段和引用一致性；可迁移旧数据 SHALL 生成备份后迁移，不可迁移数据 SHALL 隔离并给出恢复指引，不得静默丢弃。工程目录等只读投影遇到具有完整显式迁移链的旧版 Project 时 SHALL 先在内存迁移到当前模式并执行当前校验，通过后按可用工程展示，但不得因查询写回、备份或隔离记录；用户实际打开并激活该工程时 SHALL 通过 repository 的备份、迁移、复读验证和原子替换边界，将 Project 及其活动 Variant 升级到当前模式。兼容判断 SHALL 接受全部有明确迁移路径的历史模式，不得只硬编码某一个旧版本；未来版本、身份/路径不匹配、迁移失败或当前校验失败 SHALL 保留原记录并明确拒绝激活。

**FR19.8 Project catalog 缺失自愈** — *状态：已实现并通过综合 QA（2026-08-25；旧模式兼容补齐于 2026-09-12） | 优先级：P0*：当 `project-catalog.json` 缺失但持久化根目录中仍存在合法的当前模式 Project，或存在可按 FR19.7 在内存完整迁移并通过当前校验的旧版 Project 记录时，系统 SHALL 在读取开始中心工程列表前自动重建目录索引，使全部可安全识别的本地工程重新可见。重建 SHALL 只接受 ID、文件位置、schema、entity type、内部引用和显示名称均通过当前合同验证的 canonical Project 记录；单个无效、不可读、未来版本或非 canonical 文件不得被加入目录，并 SHALL 产生可审计诊断。多个合法记录若产生重复规范化名称等目录不变量冲突，系统 SHALL 拒绝自动选择并保持原记录不变。若 catalog 已存在但无法解析或合同无效，系统 SHALL 保留现场并报告损坏，不得以扫描结果自动覆盖。重建写入 SHALL 原子提交并在发布前复读验证；失败、重复启动或无可恢复工程时不得留下半写索引，也不得修改 Project、Variant 或活动工程指针。只读目录查询仍 SHALL 无写盘副作用；旧 Project 只在用户实际打开并激活后按 FR19.7 写回升级。
...
### FR30：智能助手用户请求生命周期与长会话上下文

**状态：FR30.1～FR30.22 的核心链路及稳定上下文已实现并完成离线验证；真实模型语义、缓存收益与重复性能评估未完成。2026-09-22 决策材料第一版功能回归通过，状态查询前台复验的性能设计待用户选择。** 关联 FR12、FR13、FR19、FR20；架构见 [ADR-040](adr/040-assistant-user-request-lifecycle.md)、[ADR-041](adr/041-assistant-context-compaction.md)。本节扩展用户请求管理，不改变既有翻译项目快照语义。

**FR30.1 请求身份与多问题跟踪**：一个会话可包含多个独立用户目标，一个目标可包含多个验收项并跨多个模型轮次和后台任务。系统 SHALL 持久化请求身份、目标、约束、待完成项、关联消息和执行结果。一次消息可以补充现有请求，也可以包含多个请求操作，不能把每条消息、每次模型回答或每个后台任务直接视为一个完整请求。

**FR30.2 输入接纳与关系判定**：输入 SHALL 在被确认接纳后可靠保留；连续快速发送不得丢失较早问题。寒暄、感谢及无需工具或进度跟踪的普通问答 SHALL 直接回复，保留聊天记录，不创建 UserRequest 或要求事项完成声明；需要工具、多步骤或用户明确要求跟踪的工作才进入请求流程。同一句包含问候和工作时不得丢弃工作部分。关系判定 SHALL 覆盖直接回复、新建、追问、补充约束、继续、取消及替换，支持同一消息的多个操作。目标不明确时只澄清受影响的操作；感谢、换题不得自行恢复、重开或取消旧请求。

**FR30.3 自动继续与前台秩序**：用户已于本轮明确选择“当前回答结束后继续推进旧请求，需要确认时等待用户”。当前会话中后台任务结束 SHALL 先更新原请求；新问题优先，旧请求可继续时排队进入后续轮次，不打断当前回答。用户暂停、取消、关闭会话或退出应用后不得借此规则继续自动推理；切换会话后的后台结果仍须记录原归属。

**FR30.4 完成证据**：请求完成 SHALL 核对所有验收项；问答项关联已完成的回答，执行项关联真实结果和必要的提交凭据。模型轮次结束、线程退出或模型自行声称完成均不足以证明整个请求完成。部分成功、待确认和未处理问题必须可见。

**FR30.5 修订与迟到结果**：目标或约束修改 SHALL 保留原版本及修改依据。已启动工作使用的版本必须可辨认；新约束未应用到在途工作时应明确说明。旧版本结果可保存为历史证据，未经重新核验不得满足新版本要求或自动启动新步骤。

**FR30.6 确认与控制**：确认 SHALL 关联请求版本、操作及目标；参数改变、请求取消或替换使对应确认失效。请求取消 SHALL 向其执行工作传播，并在实际收敛前展示取消中；若副作用结果未知，不得宣称已完整结束。暂停调度与后台执行暂停分别说明，不支持安全暂停时不展示已暂停。

**FR30.7 完整历史与模型上下文**：系统 SHALL 分别管理持久化消息记录和本轮模型输入；裁剪、压缩或切换模型不得删除已保存历史。入口判定可使用限量近期聊天；已完成请求仅按近期、标题相关或显式身份引用选入必要候选，不逐轮发送全部终态事项与约束。候选遗漏应显式标明，无法定位的旧目标先澄清，不能猜测创建替代工作。当前目标、有效约束、未完成项和用户待决事项应在预算内优先保留；请求摘要记录来源和覆盖范围，不能覆盖权威任务状态或用户授权。

**FR30.8 输入预算与结果回查**：模型调用 SHALL 对消息、工具定义和动态上下文一起核算输入预算，并预留输出空间；原生工具调用与结果应保持协议完整。大结果保存可追溯引用，摘要之外的内容能够按权限回查。预算不足或摘要不可用时应有明确降级，不能静默丢弃当前约束后继续执行。

**FR30.9 持久化与恢复**：请求、输入处理位置、执行关联和结果证据 SHALL 可恢复；后台结果保存不依赖当前 UI 会话。应用重启后先核对任务、提交凭据、项目身份和确认条件，再展示可继续位置；不自动重放未知结果的副作用或将历史确认当作仍有效的执行许可。

**FR30.10 可见与可纠正**：聊天区 SHALL 能查看当前目标、其他未完成目标、正在等待的原因和关联结果，提供继续、暂停推进、取消及纠正请求归属的入口。没有工作请求或待处理输入时不展示空请求管理面板；普通回复不显示事项进度。自动分类不确定或错误时允许纠正；已执行工作不能通过修改显示归属绕过所有权或修订检查。

**FR30.11 隔离与故障边界**：消息、检索材料、确认和结果 SHALL 按会话及请求归属处理；检索材料不得成为新的控制指令。保存失败时停止新的副作用接纳，保留可恢复信息和错误；重复投递、乱序事件和旧轮次回复不得重复执行或改变错误请求。

**FR30.12 验收场景**：至少覆盖同句“取消 A、补充 B、新建 C”、连续三次输入、翻译期间追问并自动继续、三个问题只回答两个、21 轮后约束保留、超长工具结果回查、修订后旧结果、审批失效、跨会话终态落盘、提交前后崩溃、任务丢失、摘要失败、上下文超预算和旧格式迁移。追加覆盖模型重新编号导致的重复请求、控制与业务调用混用、回答截断或缺少覆盖声明、同请求独立项等待、双窗口同会话唤醒、发送后切项目、跨会话同资源写入，以及主动停止后禁止自动重启。自然语言归属准确性使用独立固定语料评估，程序状态正确性使用确定性测试验证，两者不得互相替代。

**FR30.13 模型与程序职责**：模型 SHALL 提出有来源的请求变更、跟踪问答的覆盖声明，或不改变工作状态的普通回复；程序负责分配正式身份、验证、持久化并返回接纳结果，执行状态依据真实运行事件更新。普通回复正文与输入消费位置 SHALL 原子保存，重放不得重复回复。控制协议和业务执行入口分离，模型不能通过任意状态写入宣称完成，截断或被中断的回答不能满足验收项或提交普通回复；重试提案不能因模型改写编号而重复接纳。界面应区分主动停止生成、新消息技术中断、暂停推进与取消请求，用户主动停止后不自动恢复该请求。

**FR30.14 输入与调度所有权**：输入接纳时 SHALL 固定来源项目/版本及选择引用，延迟路由不读取后来切换到的 UI 目标。多窗口打开同一会话仍只有一个有效前台轮次；不同会话对同一实际资源的冲突写入须统一协调。单个待确认项只阻塞它和依赖它的工作，独立问题仍可回答；请求整体暂停或停止则阻塞全部新工作。

**FR30.15 过程可追溯与评测分离**：系统 SHALL 将请求状态变化、输入与路由处理、轮次接纳、确认、后台结果、恢复及删除意图的诊断事实与请求状态一起保存，保留操作来源、状态差异和关联引用。保存失败不能留下成功事件；重试不能重复记录同一已接纳轮次或无变化命令。过程查看是只读操作，不启动模型或改变请求。旧会话缺少记录时明确说明，不补造历史；事件不成为模型指令。对话验收 SHALL 经实际协议解析和路由归约检查结果，合成回放与真实模型采集分别统计，未采集不能报告真实模型通过。

实施状态：用户已授权实施 FR30.16～FR30.22，生产请求已接入稳定追加和独立语义摘要；2026-09-23 清理旧组装与摘要刷新入口，确定性摘录仅保留兼容读取、精确迁移校验及对应测试。相关离线回归通过，真实语义质量、缓存收益及重复性能样本仍待评估。完整证据、请求归档和显式离线附件维护继续有效；不引入云端调度、多会话无人值守自主推进、任意项目回滚或依赖语义向量检索的恢复。

<a id="fr30-stable-context"></a>

#### FR30.16：稳定的模型上下文

同一请求在模型、固定规则、工具及相关配置未改变且未压缩时，普通问答和工具续跑 SHALL 保留已发送上下文并追加新材料，不因超过固定轮数重新移动历史窗口。执行恢复由代码与 checkpoint 负责；模型只获取当前判断必需的目标、约束、活跃事项、依赖及未结算操作事实，完成记录和完整证据按需查询，不默认发送恢复账本。模型相关事实变化 SHALL 以独立序号标明有效性，不依赖消息位置或需求修订号判断新旧，不原地重写过去的事实。模型可见材料与真实用户输入分开，运行事件不得变成新用户命令。归属纠正、配置变化或协议不兼容确需重建时 SHALL 记录原因。目标、计划、checkpoint、授权、取消与完成证据规则继续遵守 FR30.1～FR30.15。

#### FR30.17：按容量压缩与任务连续性

上下文接近包含全部既有摘要、工具、当前新增材料、输出预留及余量的有效预算时，系统 SHALL 仅整理尚未摘要的原文历史，并为后续交流留出空间；普通短对话不因轮数触发压缩。已激活摘要 SHALL 按原顺序、原内容继续出现在实际模型输入中，后续新增独立摘要段，不替换、合并、改写或移除旧段，也不以仅存档代替发送。整理后 SHALL 保留当前目标、有效约束、未完成及待决事项、关键决定及理由、必要工具链和结果来源，原文保持可查。后续决定通过新事件或新摘要表达对旧决定的修订，当前状态以权威状态为准；摘要不得创建许可、修改计划进度或宣称请求完成。摘要链及必需材料无法容纳时 SHALL 进入明确的容量等待，不自动再压缩或删除旧摘要；压缩失败不得静默删掉用户约束继续执行。

#### FR30.18：准备期间的并发与取消

整理上下文 SHALL 在后台进行，用户输入、停止和关闭保持响应。生成期间的修订、归属纠正、暂停、取消、配置变化或执行资格丢失 SHALL 阻止旧候选激活。后台结果到来时保持原请求归属且只消费一次；多窗口及保存重试不得重复激活或重复发起业务操作。整理过程不占用业务计划步骤，不改变任务终态。

#### FR30.19：版本恢复与旧会话兼容

模型工作上下文 SHALL 独立保存版本、全部摘要段及来源边界，切回请求或重启后在通过既有恢复核验的前提下恢复完整摘要链，不仅恢复最新一段。旧会话 SHALL 能从现存完整证据建立新版本，保留已有摘要正文，不删除聊天或重置目标/计划；旧版本已经缺失的原文须明确标记，无法验证来源或权限的摘要保留诊断且不得注入模型输入。损坏的 head 或索引须进入恢复等待，通过既有完整备份恢复并核验原摘要链后再建立新版本，不根据孤立附件猜测活动顺序；已激活摘要正文损坏须从可验证副本原样恢复，无法恢复时明确等待处理，不重新生成不同正文冒充原摘要。必要原始证据损坏不可由摘要代替。备份、归档和清理 SHALL 保留当前及备份引用；崩溃留下的未提交派生附件不得被当成活动上下文。

#### FR30.20：稳定结果与有权限的原文回查

大工具结果首次提供给模型时 SHALL 保留足够的状态、异常和精确引用，后续不因预算变化反复改写同一条结果。完整工具调用和结果保持协议完整。用户原文、关键决策及结果 SHALL 能按请求权限分页回查，并带来源、范围、完整性或缺失诊断；查询不得接受模型任意拼接本地路径，不得把历史材料解释为新授权。来源归属纠正后旧引用不能继续绕过权限读取。

#### FR30.21：真实用量与效果可验证

系统 SHALL 区分供应商实际用量、估算与未知，按模型调用分别记录路由、执行、摘要和重试的消耗及延迟；支持时提供缓存读取和写入用量。取消或过期响应仍可产生费用，未知值不得显示为零或伪装精确价格。缓存配置不支持时基本对话仍可用且降级可识别。验收 SHALL 同时比较任务正确性、全部调用成本和延迟，不用本地前缀相同率代替真实缓存命中率。

#### FR30.22：有界失败处理与可见反馈

用户 SHALL 能辨认“整理上下文”“容量不足”和“整理失败”的状态，并显式重试；不新增每次压缩的日常确认步骤。失败、重启和自动唤醒不得无限发起摘要调用或重放业务副作用。超大旧会话的初次整理须有调用预算、可取消且可继续。关闭供应商缓存不得破坏聊天，暂停自动整理不得绕过容量检查。配置变化后的行为可辨认，但不承诺不同模型、不同账号或缓存过期后仍有复用。

验收补充：覆盖 50 轮短对话、巨大结果回查、三次压缩及重启后全部旧摘要原样进入模型输入、新段只覆盖尚未摘要原文、后续决定修订、摘要链超限时不自动删改旧段、目标/计划状态不变、压缩期间用户修订和双窗口竞争、旧会话与坏附件、模型/工具切换、取消及 usage 缺失；离线协议/状态验收与真实语义/费用评估分别报告。具体实现顺序见 [assistant-context-compaction](../plans/assistant-context-compaction/plan.md)。首期已复用现有 AI 服务生成语义摘要，原生压缩接口和向量记忆不构成本轮依赖；技术选择见 ADR-041，实际测试与未完成验收见实施验证报告。

#### FR30.23：取消默认保留与显式撤销入口

用户取消 SHALL 立即阻止本轮新的模型/节点推进，已正式提交结果默认保留，不弹“保留或撤销”的选择框。未提交的临时修改依工具既有合同清理；实际收尾前显示正在停止，不提前宣布成功。停止后提供“本轮已停止”提示与“撤销本轮修改”入口，只有用户点击才请求撤销已提交修改。清理失败 SHALL 明确报告，不以普通已取消掩盖。

#### FR30.24：工作轮次与撤销边界

工作轮次 SHALL 以真实用户输入为来源，包含该输入引发的模型回合、工具和 DAG 执行；不得以一次 API 调用或整个长期请求替代。修改归属须可证明，不从最后一条消息猜测。助手写入缺少轮次归属或必需撤销服务时 SHALL 在执行前明确失败，不静默退回无记录写入；旧任务仍可读取，新的真实输入经路由建立新轮次后可继续修改。独立非助手工具不强制建立助手轮次；有真实归属但领域上不可安全撤销的操作仍按已声明限制执行。撤销 SHALL 保护无关后续编辑、核对目标身份和实际前后值；冲突、外部不可逆效果、未知结果或凭证缺失须先说明限制，不将部分恢复称为完整成功。对话和审计保留。没有已提交修改时明确说明，无须伪造恢复操作。

#### FR30.25：结果提交与界面职责

控制工具调用的成功、失败和取消 SHALL 由应用层统一接纳并保存，再发布内存/界面结果。相同调用重复结果幂等，冲突显式拒绝；父调用、请求归属及当前资格须核验。GUI 不执行完整账本序列化、持久保存或等待后台存储锁；切换/关闭和迟到通知不得改变错误回合或重复终结。保存失败不能留下可继续使用的成功结果。

#### FR30.26：撤销与 DAG checkpoint 隔离

本轮撤销 SHALL 核对业务提交和回滚事实，并使相关旧许可与图恢复资格失效；不通过加载旧 checkpoint 撤销业务数据，不自动重放工具。checkpoint 文件存在不等于任务可恢复，失败/取消节点不能按 completed 名称当作业务成功。本轮不新增未经验证的跨进程自动图恢复。实现与验收见[综合计划](../plans/assistant-task-context-consistency/plan.md)。产品交互已确认，技术实现状态以计划为准。
...
## 助手长期记忆需求（草稿，2026-09-13）

<a id="fr31-assistant-memory"></a>

### FR31：智能助手自动长期记忆

- **状态**：草稿，尚未实现。用户已明确日常记忆应自动保存，无需逐条点击或审批；其余范围与验收取舍是当前规划建议，不因补齐本文档而视为全部已确认。
- **优先级**：P1；隔离、纠错、撤销和业务权限边界为该功能发布前必须通过的门禁。
- **目标**：助手在日常交互中积累可靠的个人偏好、项目约定和经验，在新会话中正确使用，减少用户重复说明；用户保有查看、纠正、忘记和关闭能力。
- **范围**：本机个人与项目记忆、即时与后台自动写入、跨会话召回、来源展示、修订、遗忘、生命周期和效果验证。必要时进一步限定项目版本和目标语言。
- **范围外**：跨机器或多用户共享、自动修改 Skill/系统规则、替代术语库或翻译记忆、保存执行许可、无人值守持续学习、自动恢复已退役的旧记忆数据。索引、存储和模型适配等技术选择由 ADR/Plan 负责。
- **依赖**：FR19 的 Project/Variant/Session 生命周期、FR20 的任务执行事实、FR30 的请求权限与长会话上下文。FR7.13.3 / FR10.5 保持退役；FR31 是独立新需求。
- **追溯**：[ADR-042](adr/042-assistant-durable-memory.md)（提议）→ [实施计划与流程图](../plans/assistant-durable-memory/plan.md)（草稿，10 个 Story）。本节是长期记忆行为与验收的权威需求来源。

#### FR31.1：自动保存与内容筛选

系统 SHALL 自动识别有可靠来源的长期偏好、项目约定和可复用经验，校验后保存并供后续会话使用；无需用户逐条点击保存、采纳或确认，也不要求每次使用“记住”口令。明确要求、长期偏好和纠正 SHALL 及时处理，较完整的任务经验 MAY 在后台整理后生效。纯猜测、临时要求和无法核验的信息不得发布为有效记忆。

验收：

- 用户说“这个项目一直沿用旧版人物称谓”，无需点击即可在该项目的新会话中引用此约定；“这次简单说”不成为永久偏好。
- 明确“记住”保存成功时展示可撤销回执；保存失败不得回复“已记住”。普通自动整理不产生逐条确认弹窗或候选待办列表。
- 空内容、只有寒暄、来源缺失和无法确定含义的输入不产生有效条目；包含否定或条件的约定不能被归纳成无条件肯定结论。

#### FR31.2：作用域与访问隔离

系统 SHALL 根据当前用户配置身份、Project 及必要的 Variant/语言确定适用范围，默认采用能证明的最窄范围。可明确判断为跨项目个人偏好的信息 MAY 自动保存为个人记忆。范围未知时不得猜测或扩大；未经授权的其他项目记忆不得通过搜索、详情、引用或缓存返回。

验收：

- 同名不同 ID 的项目不混用约定；项目移动路径但身份不变时可以继续关联，复制为新项目不默认继承记忆。
- 个人偏好可跨本机项目使用，项目/版本/语言约定只在相应范围生效；没有项目身份时不保存项目特定约定。
- 收窄范围后，旧范围停止使用；跨范围转移中途失败时呈现真实结果并可恢复，不能悄悄恢复已撤销的宽范围。

#### FR31.3：来源可追溯与业务事实边界

每条有效记忆 SHALL 保留内容、来源、适用条件、更新时间及修订关系，并区分用户表达、用户反馈与模型归纳。助手记忆 SHALL 不拥有术语译名、译文、业务配置、执行许可或任务完成状态的权威副本。需要改变这些数据时 SHALL 走现有业务功能。

验收：

- 用户能查看一条记忆来自哪次交互或哪个实际结果；无原文访问权、来源缺失或已删除时如实展示状态，不扩大历史访问权限。
- “某地名统一译为雪漫”交由术语功能处理；助手可记住采用某套译名的理由或资源引用，不能另外维护可写的权威译法。
- “工具已启动”不被记成“工作成功完成”；外部文件和工具结果中的命令式文字不升级为用户指令或执行授权。

#### FR31.4：跨会话召回与当前任务一致性

新请求 SHALL 在当前范围与输入容量内使用相关有效记忆，复杂经验可按需查询。无命中时 SHALL 正常继续，不编造过去经历。记忆的加入、查询和整理 SHALL 遵守 FR30 的稳定上下文、完整工具协议与权限边界，不挤掉当前必要目标和约束。

验收：

- 新会话可使用适用偏好；改变项目、版本或语言后重新确定范围，不能沿用不适用材料。
- 无关记忆新增不导致普通续跑反复重排旧材料；记忆过多或过长时使用有界结果，完整保留入选条目的否定和适用条件。
- 当前用户明确要求优先于历史偏好；读取记忆不会自动创建业务请求、批准计划、完成验收项或触发写回。

#### FR31.5：自动更新、冲突与幂等

系统 SHALL 自动去重、补充证据并处理可确定的用户纠正；同一操作重复执行不能重复写入，迟到的旧结果不能覆盖新修订。无法确定的新旧冲突 SHALL 暂不更新，保留原有效信息，不把内部判断转为用户记忆审批。只有冲突影响当前任务结果时，才按正常业务交互澄清事实。

验收：

- 用户明确更正约定后，下一次可分派的回答采用新有效版本，并能区分历史决定与当前决定。
- 重复回调、重复整理或等价信息不产生重复条目；自动合并不删除条件、不扩大范围、不把推断改称用户原话。
- 两个窗口并发更新和整理期间用户纠正时，旧结果不能覆盖新决定；普通模糊冲突不弹“是否保存记忆”。

#### FR31.6：忘记、来源删除与活动上下文

用户要求忘记后，系统 SHALL 停止该记忆的后续召回与同源再提取，并使依赖它的未发布结果失效。忘记已保存记忆、修改记忆和删除原会话 SHALL 具有明确不同的用户语义；不得声称已撤回模型先前收到的内容或回滚已执行的业务操作。

验收：

- 忘记后新会话无法再次召回该条目；后台整理、重启、重复删除和受控备份恢复不会使它复活。新的明确保存要求可重新建立对应记忆。
- 来源会话或项目删除期间停止相关新学习和召回，并失效相应派生记忆；删除部分失败时呈现处理中/失败状态，恢复后继续收敛。
- **当前草稿交互取舍**：已使用被撤销记忆的请求停止后续模型调用，保留原任务及真实结果，提供不继承旧历史、摘要或许可的干净会话入口；不改写 FR30 已要求保留的旧摘要。该交互仍待评审，不等于已确定采用原请求无感续跑。
- 已发出的模型调用可能无法撤回，但迟到建议不得继续驱动新的业务操作；已运行任务的真实结果继续按原生命周期保存。

#### FR31.7：可查看、可纠错、可关闭

系统 SHALL 提供个人与当前项目记忆的查看、来源、最近自动更新、编辑、范围调整和忘记能力。默认自动记忆，管理入口用于检查和纠错，不要求用户维护候选审批队列。使用记忆与允许自动学习 SHALL 可独立关闭，并能识别当前会话覆盖设置。

验收：

- 普通使用无需打开管理界面即可产生并使用记忆；用户可主动修改或删除，且能通过键盘完成操作。
- 关闭学习停止后续提取但不自动删除已保存条目；关闭读取停止后续引入，已经使用的活动上下文按 FR31.6 处理，不能显示虚假的“已清空上下文”。
- 空列表、加载失败、权限不足和来源不可访问有可区分反馈；项目切换、长文本和大列表不造成界面失去响应。

#### FR31.8：后台整理、取消与费用透明

后台整理 SHALL 默认启用、可关闭，只处理已持久化、当前允许访问且允许用于学习的材料；提取、校验后自动保存，不经过用户采纳。整理 SHALL 有工作量、并发、调用预算和重试上限，优先保障用户正在进行的对话与业务任务，复用用户现有 AI 服务并如实记录用量。

验收：

- 普通交互结束或闲置后可靠信息自动生效；连续输入不反复触发对同一材料的整理，已成功处理的来源不重复消费模型额度。
- 暂停、关闭窗口、来源删除或设置变化后，旧后台结果不能重新写入；网络超时和格式错误不阻塞聊天、不改变业务任务成功/失败状态。
- 失败保留可恢复进度并有界重试，部分成功如实保留；没有可用服务时明确诊断，不自动切换计费服务。无法取得真实用量时标为未知或估算，不能伪装精确费用。

#### FR31.9：持久化、恢复与旧数据兼容

有效记忆及其修订/撤销状态 SHALL 在正常重启后保持一致，多窗口和多进程写入不能互相覆盖或造成半完成数据。未知格式、损坏、只读环境和恢复失败 SHALL 有明确反馈。新功能不得读取、迁移或删除 FR7.13.3 已退役的旧用户记忆文件，也不得改变既有翻译记忆数据。

验收：

- 保存成功后重新打开应用仍可读取相同有效内容；并发、崩溃或写入失败不能让旧状态覆盖较新的用户纠正。
- 全新请求在记忆不可用时可明确提示后无记忆继续；已消费记忆且无法核验撤销状态的请求不继续发送旧材料。
- 受控恢复保留较新的撤销事实；无法证明时禁用记忆并提示处理。应用外替换整个数据目录不属于任意文件系统回滚检测保证。
- 关闭或升级记忆功能后，既有会话、摘要、术语、译文与运行任务仍遵守各自的兼容和恢复合同。

#### FR31.10：发布验收与效果可验证

首个用户版本 SHALL 包含自动保存、跨会话使用、自动更新、纠正和忘记闭环，不能以仅显式保存或待采纳列表代替。验收 SHALL 覆盖实际应用集成链路、负路径、恢复和真实模型效果；纯离线测试通过不等于真实模型收益已验证。

验收：

- 在“普通交流→自动记住→关闭重开→新会话使用→更正→长会话压缩→忘记”旅程中，全程不要求点击保存或采纳；使用受控合成资料覆盖跨项目、版本和语言变化。
- 隔离泄露、记忆带来的权限升级、同源复活和业务工具重放必须为零；同时覆盖空输入、无效输入、重复操作、部分失败和取消。
- 在相同模型、任务、预算和工具条件下比较无记忆、仅显式记忆与自动记忆，记录最终任务正确性、错误记忆采用、纠正生效、检索效果、额外用量及耗时。具体数据集、样本量和性能初值由实施计划维护；未执行项目明确标为未执行。
- UI 响应、取消反馈、存储容量与查询性能 SHALL 满足既有适用非功能需求及计划定义的测量门槛，报告环境、冷热状态、重复样本与计时边界，不使用未测量的性能结论。

#### FR31 的未决项与当前假设

自动保存、无需逐条确认是用户已明确的交互方向。当前按本机保存、项目优先、默认学习和读取开启规划；忘记后受影响请求采用干净会话入口仍是草稿取舍。跨机器共享、向量召回和原请求无感遗忘不属于首期依赖；如改变范围，应先更新本节，再对齐 ADR 和计划。
...
| 2026-09-12 | 补充 FR19.7～FR19.8：全部具有明确迁移链且经当前合同验证的旧版 Project 可在只读目录中安全展示，缺失 catalog 时可参与只读自愈；只有用户实际打开并激活后才备份并原子升级 Project/Variant，未来、损坏或身份不符记录保持原样并拒绝激活 | 用户要求旧版本读取验证通过后自动升级，并通过 /bm-pilot 补齐开发前流程 |
| 2026-09-06 | 将 FR5.18 从单一“本体汉化适配”提升为通用“译名方案”，明确可承载地区/社群、团队/发行、系列旧译/新译与整合包约定；补充工作台和 AI 配置共享可见选择、术语来源分离及任务启动冻结合同 | 用户要求扩展未来使用边界并通过 /bm-pilot 实现 |
