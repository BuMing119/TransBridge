...
**状态：已实现（2026-09-12，离线集成验证通过；真实模型语料评估未执行）**。关联 FR12、FR13、FR19、FR20；架构见 [ADR-040](adr/040-assistant-user-request-lifecycle.md)。本节扩展用户请求管理，不改变既有翻译项目快照语义。
...
范围外：本轮不实施代码；首期不引入云端调度、多会话无人值守自主推进、任意项目回滚或依赖语义向量检索才能工作的请求恢复。其他设计默认值仍为提议。
