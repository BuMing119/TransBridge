...
- 状态：S01～S08、S10～S11、S13 本地实现与离线验证完成；2026-09-22 第一版相关回归 1,263 项通过；S12 已纳入[综合实施计划](../assistant-task-context-consistency/plan.md)的应用层结果提交和后台接线；S09 真实模型收益与重复性能样本仍未完成。
...
本轮改进（2026-09-22，用户通过 bm-pilot 授权第一版）：执行恢复仍由代码和 checkpoint 负责；
助手只获取下一次决策/回答所需的材料。用户已明确选择保留计划完成后的结果解释回合，
本版不改完成回调是否启动模型的规则。遇到协议/设计不一致直接报告，不新增循环重试、
静默退回全量历史或丢弃必需材料的兜底。

- [x] S11 合同：按 admission 的问答/执行目的保留目标、约束、活跃事项、依赖与未结算操作；完整完成详情留在权威记录。
- [x] S11 实现与验证：模型材料实质变化才追加，使用独立单调序号；旧版无序号状态明确失效，压缩重排及 A→B→A 不改变有效性。
- [x] S12：后台分页查询、应用层结果提交与取消终结已实现；综合计划 S03/S04 的 Qt 持锁响应与真实回执一致性回归通过，真实模型收益仍属 S09。
- [x] S13：助手 OpenAI 使用稳定 key 和自动历史缓存；相关回归 1,263 项通过，文档与本次增量已记录；真实服务端缓存未测。

落点：`state_projection.py`、`models.py`、`projection.py`、`compaction.py`、独立状态查询协议/服务/UI 接线、
`assistant_prompt_cache.py`，以及对应上下文、供应商、请求生命周期测试。新策略不扩充 GraphExecutor，
不让模型恢复节点或重新发放许可。依赖为合同 → 材料/查询及版本 → 联合验证；缓存可独立验证。

验收：重复唤醒/内部 sequence/已完成 receipt 变化不追加模型材料；完成详情增长不导致活动材料等比例增长；
未决/失败/未知结果与依赖状态精确保留；普通追加前缀不变；压缩、配置切换、重开后最大序号仍为当前材料；
查询不接收任意请求 ID/路径，取消或迟到结果不能交付；无静默修复损坏版本；服务端缓存收益单独标为未测。
保留原始选择范围，避免猜测用户输入时的筛选/选择语义；超大活跃工作集和全部摘要链仍可能明确容量等待。

迁移：新状态消息自描述版本（`request_decision_context`/`state_seq`）；旧 `current_request_state` 只读识别。
普通追加不改旧字节，首次新材料明确替代旧格式；工具 schema 变化允许一次配置重建。旧程序不能解读新状态，
回退使用升级前完整备份；不承诺降级兼容。摘要链继续原样发送，不在本版引入递归摘要或通用增量重放。

S12 完成范围：原 `request_state_binding.display` 的前台全 section 复验与同步保存已移除；
8000 字分页之外的核验/保存由后台应用用例统一裁决，前台仅投影持久回执。
同时处理取消/提交竞争和模型调用组完整性，保留资格检查，不添加旧状态缓存兜底。
已运行 Qt 持锁响应测试；整个 UI 的其他旧同步读取并非本版全部消除，剩余项见综合计划。
...
活动输入为：固定规则 → 已有全部摘要 → 本版本必要决策材料及保留原文 → 后续新消息/决策材料。压缩将最高 state_seq 的材料放在摘要之后；普通续跑不回写旧字节，旧无序号状态与较小序号只是历史。旧系统提示版本仍在 transcript 中，模型使用最新固定规则。执行恢复账本不投给模型，必要材料仍随活跃工作量增长，不承诺总上下文永久有界。
