...
1. 检查最高 state_seq 的必要决策材料与当前权威投影一致；不是要求完整执行记录进入模型。旧版材料只供明确迁移读取。
...
6. 程序分配新 summary_id 和覆盖范围，原样继承所有旧段；将最高 state_seq 的决策材料放到摘要之后，再保留原文和后续事件。旧材料即使排在后面也不能覆盖最大序号；A→B→A 按独立事件身份分组，不以内容 hash 识别最新事件。
