...
1. **状态检测**：扫描 `.claude/bm_config/`、目录结构、索引文件，判断哪些已存在、哪些缺失
...
| 检测项 | 路径 |
|--------|------|
| 行为配置 | `.claude/bm_config/pilot.json` |
| 路径配置 | `.claude/bm_config/paths.json` |
...
**AskUserQuestion 4 — defaults.complexity**：
- 问题："复杂度判定策略？"
...
**AskUserQuestion 7 — timeout.per_stage_minutes**：
- 问题："单阶段超时提醒（分钟）？"
...
根据用户选择生成 `.claude/bm_config/pilot.json`。若目录不存在则先创建。

### 4.2 写入 paths.json

根据用户选择（或默认值）生成 `.claude/bm_config/paths.json`。
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
配置文件:
  [✓] .claude/bm_config/pilot.json
  [✓] .claude/bm_config/paths.json
