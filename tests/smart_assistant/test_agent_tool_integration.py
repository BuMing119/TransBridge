...
class TestFullWorkflowChain(unittest.TestCase):
    ...
    def test_full_workflow_filter_search_select_edit_mark(self):
        ...
        r5 = _tool_edit_translation(
            {"entry_id": target, "new_translation": "集成测试译文", "new_stage": 1}, ec,
        )
...
class TestLabelSystem(unittest.TestCase):
    ...
    def test_label_full_lifecycle(self):
        ...
        r3 = _tool_manage_entry_labels(
            {"action": "assign", "name": "需要校对", "entry_ids": ["entry_000", "entry_001"]}, ec, self.collection,
        )
        self.assertTrue(r3.success)
        ...
        r6 = _tool_manage_entry_labels(
            {"action": "unassign", "name": "需要校对", "entry_ids": ["entry_000"]}, ec, self.collection,
        )
        self.assertTrue(r6.success)
...
class TestSecurityGuardrails(unittest.TestCase):
    ...
    def test_execute_with_guardrails_perm_denied(self):
        ...
        result = execute_with_guardrails(spec, {"path": "test.esp"},
                                         ExecutionContext(app_context=self.ctx))
        self.assertFalse(result.success)
    ...
    def test_execute_with_guardrails_allows_read(self):
        ...
        result = execute_with_guardrails(spec, {"stages": [0]},
                                         ExecutionContext(app_context=self.ctx))
        self.assertTrue(result.success)
    ...
    def test_execute_with_guardrails_input_validation(self):
        ...
        result = execute_with_guardrails(spec, {"path": "../secrets/config.esp"},
                                         ExecutionContext(app_context=self.ctx))
        self.assertFalse(result.success)
    ...
    def test_writer_tools_have_admin_permission(self):
        ...
        for spec in writer_specs:
            self.assertEqual(spec.permission, "admin",
                             f"Writer tool {spec.name} should have admin permission")
            self.assertTrue(spec.require_confirmation,
                            f"Writer tool {spec.name} should require confirmation")
    ...
    def test_parser_tools_have_write_permission(self):
        ...
        for spec in parser_specs:
            self.assertEqual(spec.permission, "write",
                             f"Parser tool {spec.name} should have write permission (Story 24)")
...
class TestToolResultV2(unittest.TestCase):
    ...
    def test_partial_ok(self):
        r = ToolResult.partial_ok("部分成功", data={"ok": 3},
                                  failed_items=[{"id": "x", "reason": "not found"}])
        self.assertTrue(r.success)
...
class TestFilterEntries(unittest.TestCase):
    ...
    def test_filter_by_search_text(self):
        results = filter_entries(self.collection,
                                  {"search_query": "Original text 0", "search_field": "text"})
        self.assertGreater(len(results), 0)
        for e in results:
    ...
    def test_filter_by_search_id(self):
        results = filter_entries(self.collection,
                                  {"search_query": "entry_000", "search_field": "id"})
    ...
    def test_combined_stage_and_search(self):
        results = filter_entries(self.collection,
                                  {"stage": [1], "search_query": "Original", "search_field": "text"})
        self.assertGreater(len(results), 0)
        for e in results:
...
class TestAgentRegistry(unittest.TestCase):
    ...
    def test_namespace_wildcard_expansion(self):
        ...
        for tool in expanded:
            self.assertIn(tool, editor_names,
    ...
    def test_agent_tools_are_expanded(self):
        ...
        for tool in editor_agent.tools:
            self.assertFalse(tool.endswith(":*"),
                            f"Tool '{tool}' should not be a wildcard after expansion")
