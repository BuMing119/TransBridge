...
def test_build_tool_directory_returns_string():
    ...
    assert "Available tool directory" in result
...
def test_build_tool_directory_default_first():
    result = ToolRegistry.build_tool_directory()
    first_ns_line = [line for line in result.split("\n") if line.startswith("[")][0]
...
def test_build_tool_help_overview():
    ...
    assert "Tool overview" in result
...
def test_build_system_prompt_has_layered_structure():
    ...
    assert "Native tool calls" in prompt
    assert "get_tool_help" in prompt
    assert "Tool routing" in prompt
    assert "Available tool directory" in prompt
...
def test_build_system_prompt_uses_native_calls_not_text_json():
    from transbridge.smart_assistant.prompts import build_system_prompt

    prompt = build_system_prompt()
    assert "use the API's native tool-calling mechanism" in prompt
    assert '"mode": "plan"' not in prompt
    assert '"thought"' not in prompt
    assert "输出格式" not in prompt


def test_build_system_prompt_does_not_repeat_full_tool_schemas():
    from transbridge.smart_assistant.prompts import build_system_prompt

    prompt = build_system_prompt()
    assert "| Parameter | Type | Required | Description |" not in prompt
...
def test_build_system_prompt_has_routing_table():
    ...
    prompt = build_system_prompt()
    assert "Choose the tool group" in prompt
...
def test_get_tool_help_execute_empty():
    ...
    assert "Tool overview" in result.data["help"]
