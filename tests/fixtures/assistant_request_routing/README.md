...
`corpus.json` 是人工编写的合成中文多轮情境。包含补充约束、进度追问、新问题、取消换任务、等待确认时插话、引用取消命令、歧义目标、路由期间版本变化和多指令，另有 3 个仅用于回放的协议边界场景。

`history` 便于人工审阅前后语境，不会偷偷增加到模型提示中。实际输入严格调用生产 `routing_messages()`，使用已有请求快照和当前输入。`visible_requests` 模拟模型看到旧版本、程序应用时版本已更新的竞态；`requests` 是实际应用时的权威状态。语料目前是多轮场景的单次路由截面，不覆盖后台调度、工具执行、审批点击或整段对话的端到端模型表现。
...
`origin` 是记录者声明，摘要只能校验输入一致性，不能证明真实供应商来源。记录包含模型、供应商和采集时间；请保留可信的采集过程。合成结果与模型结果分开计数，缺失结果标记 `not_captured`。只有全部 9 个可调用场景都具有通过的真实模型记录时，`live_acceptance` 才为 `passed`；3 个协议边界场景不计入真实模型通过率。
