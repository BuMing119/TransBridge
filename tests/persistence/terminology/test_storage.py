...
def test_existing_schema_zero_is_backed_up_before_migration(tmp_path: Path) -> None:
    ...
    try:
        ...
        opened.connection.close()
    backup = next(paths.backup_directory("project-1").glob("schema-v0-to-v4-*.sqlite3"))
    ...
    try:
        evidence = history.connection.execute(
            ...
            "WHERE from_version = 0 AND to_version = 4"
        ).fetchone()
    finally:
...
def test_schema_one_migration_backfills_frozen_sections_and_artifact_revision(tmp_path: Path) -> None:
    ...
    try:
        assert migrated.storage_state.schema_version == SCHEMA_VERSION == 4
        ...
        assert tuple(TerminologyPaths(tmp_path).backup_directory("project-1").glob("schema-v1-to-v4-*.sqlite3"))
    finally:
        migrated.close()
