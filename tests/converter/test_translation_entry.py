...
class TestTranslationEntry:
    ...
    def test_creat_from_eet_entry(self):
        ...
        assert entry.id == "TestEdid"
        assert entry.key == "INFO:NAM1"
        assert entry.original == "Original text"
        assert entry.translation == "Translated text"
        assert entry.stage == 0  # status != 99
    ...
    def test_create_from_plugin_entry(self):
        ...
        plugin_string = Mock()
        plugin_string.type = "INFO NAM1"
        plugin_string.editor_id = "TestEditor"
        plugin_string.form_id = "000123"
        plugin_string.string = "Original plugin text"

        entry = TranslationEntry.create_from_plugin_entry(plugin_string)

        assert entry.id == "TestEditor:000123"
        assert entry.key == "INFO:NAM1"
        ...
        # 测试没有 type 属性的情况
        plugin_string_no_type = Mock()
        plugin_string_no_type.editor_id = "TestEditor"
        plugin_string_no_type.form_id = "000456"
        plugin_string_no_type.string = "Another text"
        del plugin_string_no_type.type  # 删除 type 属性

        entry_no_type = TranslationEntry.create_from_plugin_entry(plugin_string_no_type)

        assert entry_no_type.key == "UNKNOWN"
