...
启动时若 `.dsh/bm_config/pilot.json` 或 `.dsh/bm_config/paths.json` 不存在或内容为空，则**自动调用 `/bm-init`** 进行交互式初始化，等待初始化完成后再继续执行本 skill 的后续步骤。

## 配置文件

启动时必须读取 `.dsh/bm_config/pilot.json` 和 `.dsh/bm_config/paths.json`。
...
所有编码阶段的 skill 调用必须使用 `config.dev_skill` 配置值（如 `skill(config.dev_skill, args="<feature>")`），不得硬编码。
...
用户输入需求
    │
    ▼
[步骤 1: 复杂度评估 + 路线确认]  ← 唯一一次全程确认（ask_user_question）
    │ 用户确认路线后，进入自动循环
    ▼
[步骤 2: 自动循环] ──────────────────────────────────────┐
    │                                                     │
    ...
    │   ├─ 各 skill 内部的 ask_user_question 正常触发        │
    │   │   （预沟通确认、写后确认 — 用户正常交互）         │
...
通过 `ask_user_question` 呈现评估结果和自动驾驶路线，用户确认后进入自动循环：
...
5. **自动修复**：
   - 门禁失败 → pilot **自动调用** `/bm-chronicle`（`skill` 工具）
...
```
skill("bm-analyze", args="<用户需求描述>")
skill("bm-arch", args="<从用户需求和扫描上下文拼接的参数>")
skill("bm-plan", args="<feature-name>")
skill(config.dev_skill, args="<feature>")
skill("bm-chronicle", args="记录 <epic>/<story> 的增量")
skill("bm-qa", args="<feature>")
```
...
| **预沟通确认** | bm-arch 步骤 4、bm-plan 步骤 4、bm-story 步骤 2 | 各 skill 内部的 ask_user_question 正常呈现，用户选择后继续 |
...
- **启动时读取 `.dsh/bm_config/pilot.json`**：所有配置项覆盖默认行为
