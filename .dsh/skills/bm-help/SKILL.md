...
> **调用方式说明**：本套 BM skill 在 DeepSeek Harness 下通过 `skill` 工具按名称调用（如 `skill(name="bm-dev", ...)`），而非 Claude Code 的 `/bm-xxx` 斜杠命令。下文所有 `/bm-xxx` 均指「调用同名 skill `bm-xxx`」；用户以自然语言说出需求时，由主会话按本指南自行选择并调用对应 skill。
...
| 9 | `/bm-council` | 技术议会 | 按需召集领域专家，双轮辩论（独立分析→交叉辩论），对话式建议 |
...
| 技术选型/方案评审/问题诊断/架构决策需要多视角建议 | `/bm-council` | 自动匹配领域专家，双轮辩论（R1独立分析+R2交叉辩论），对话式建议 |
...
- **核心配置**：生成 `.dsh/bm_config/pilot.json`（行为配置：dev_skill、开发者信息、阶段跳过策略）和 `.dsh/bm_config/paths.json`（顶层目录配置）
...
- **作用**：按需召集领域专家进行双轮辩论（独立分析 → 交叉辩论），为用户提供多视角决策建议
- **何时用**：面临技术选型、方案完成后想多角色把关、遇到复杂 bug 需要诊断、架构决策需要多方意见、任何"拿不准"的技术问题时
- **专家组**：核心组（架构师/开发者/质量专家）默认参与；扩展角色（安全/性能/产品/数据）按议题关键词自动匹配；议会规模 3-5 人，用户确认后方可召集
- **双轮辩论机制**：Round 1 各专家独立分析（互不知晓彼此存在）→ Round 2 交叉辩论（匿名化审阅所有 R1 观点、识别共识/分歧/补充遗漏）→ 主席汇总
- **产物**：对话式建议（共识区/分歧区/建议矩阵/议长总结/后续步骤），不写文件
- **核心规则**：不写文件、不强制结论、最终决策权归用户；议会组成必须经用户确认；R1 必须保证独立性、R2 观点匿名化
...
BM skill 流程使用 `.dsh/bm_config/` 目录下的两份 JSON 配置：

| 配置 | 路径 | 说明 |
|------|------|------|
| 行为配置 | `.dsh/bm_config/pilot.json` | dev_skill（`bm-dev` 或 `bm-dev-serial`）、开发者信息、阶段跳过策略（story_detail/council_review）、超时设置 |
| 路径配置 | `.dsh/bm_config/paths.json` | 顶层目录映射（docs_dir、plans_dir、changelogs_dir 等），子路径由各 skill 自行拼接 |
