...
class TranslationController:
    ...
    def start_polish(self, args: dict, ctx, collection) -> ToolResult:
        ...
        def _run():
            try:
                ...
                import time
                from src.transbridge.smart_assistant.tools.tool_proofreader import set_last_report
    ...
    def _get_profiles(self) -> dict[str, str]:
...
def _tool_start_polish(args: dict, ctx, collection) -> ToolResult:
    return _get_translator_controller().start_polish(args, ctx, collection)


def _tool_stop_task(args: dict, ctx) -> ToolResult:
    return _get_translator_controller().stop_task(args, ctx)


def _tool_get_task_status(args: dict, ctx) -> ToolResult:
    return _get_translator_controller().get_task_status(args, ctx)


def _tool_get_translation_config(args: dict, ctx) -> ToolResult:
    return _get_translator_controller().get_translation_config(args, ctx)


def _tool_set_translation_config(args: dict, ctx) -> ToolResult:
    return _get_translator_controller().set_translation_config(args, ctx)


def _tool_set_term_config(args: dict, ctx) -> ToolResult:
    return _get_translator_controller().set_term_config(args, ctx)


def _tool_set_scope(args: dict, ctx) -> ToolResult:
    return _get_translator_controller().set_scope(args, ctx)


def _tool_get_scope_preview(args: dict, ctx) -> ToolResult:
    return _get_translator_controller().get_scope_preview(args, ctx)
