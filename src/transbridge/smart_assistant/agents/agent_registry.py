...
class AgentRegistry:
    ...
    def init_presets(cls) -> None:
        ...
        cls.register(AgentSpec(
            ...
            system_prompt="You are the TransBridge translation engine. Follow approved glossary terms strictly and preserve all source formatting tags.",
        ))
        cls.register(AgentSpec(
            agent_id=AGENT_PROOFREADER,
            name="校对 Agent",
            role="You are a professional translation proofreading agent. Check translation consistency and formatting and perform LLM post-processing when requested.",
            namespace="proofreader",
            tools=["proofreader:*", "translator:get_task_status", "translator:stop_task"],
            skills=[],
            system_prompt="You are the TransBridge proofreading engine. Check translation quality and report exact locations and recommendations for terminology or formatting issues.",
        ))
        cls.register(AgentSpec(
            agent_id=AGENT_ORCHESTRATOR,
            name="编排 Agent",
            role="You are a translation orchestration agent. Analyze user requests, break complex work into subtasks, and dispatch each subtask to the appropriate agent.",
            namespace=None,
            ...
            skills=[],
            system_prompt="You are the TransBridge orchestration engine. Analyze user intent and create an execution plan. Use these meta-tools to describe sub-agent dispatch: parse/manage_entries/translate/run_postprocess/sync_paratranz/write/query_state.",
        ))
        ...
        cls.register(AgentSpec(
            ...
            name="解析 Agent",
            role="Parse translation source files (ESP/EET/XT/SST/JSON/Strings) and load them into translation collections.",
            namespace="parser",
            tools=["parser:*"],
            skills=[],
            system_prompt="You are the TransBridge parsing engine. Parse translation source files from the paths supplied by the user.",
        ))
        cls.register(AgentSpec(
            agent_id=AGENT_EDITOR,
            name="编辑 Agent",
            role="Manage translation entries by filtering, searching, selecting, editing translations, setting stages, and managing labels.",
            namespace="editor",
            tools=["editor:*"],
            skills=[],
            system_prompt="You are the TransBridge editing engine. Filter, edit, and mark translation entries as requested by the user.",
        ))
        cls.register(AgentSpec(
            agent_id=AGENT_PARATRANZ,
            name="ParaTranz Agent",
            role="Interact with ParaTranz for project management, uploads, downloads, terminology synchronization, and difference comparison.",
            namespace="paratranz",
            tools=["paratranz:*"],
            skills=[],
            system_prompt="You are the TransBridge ParaTranz integration engine. Manage cloud translation projects and terminology.",
        ))
        cls.register(AgentSpec(
            agent_id=AGENT_WRITER,
            name="写回 Agent",
            role="Write translation results back to source files (ESP/EET/XT/Strings). User confirmation is required before writing.",
            namespace="writer",
            tools=["writer:*"],
            skills=[],
            system_prompt="You are the TransBridge write-back engine. Write-back operations require administrator permission and user confirmation; execute them cautiously.",
        ))
